...
const emailExample = [{
    ...
    body:
        <div>
            ...
            <p>Knowledge nay estimable questions repulsive daughters boy. Solicitude gay way
                unaffected expression for.
                His mistress ladyship required off horrible disposed rejoiced. Unpleasing pianoforte
                unreserved as oh he
                unpleasant no inquietude insipidity. Advantages can discretion possession add
                favourable cultivated
                admiration far. Why rather assure how esteem end hunted nearer and before. By an
                truth after heard going
                early given he. Charmed to it excited females whether at examine. Him abilities
                suffering may are yet
                dependent. Barton did feebly change man she afford square add. Want eyes by neat so
                just must. Past
        ...
        return (
            <div className={`inbox${openMailboxes ? ' inbox--show-mailboxes' : ''}`}>
                <div className="inbox__mailbox-list">
                    ...
                    {mailboxes.map((m, i) => (
                        <button className="inbox__list-button" key={i}
                                onClick={e => this.onMailBox(i, e)}>
                    ...
                    {labels.map((l, i) => (
                        ...
                        >
                            <span
                                className={`inbox__label-color inbox__label-color--${l.color}`}/>{l.title}
                        </button>
                    ))}
                </div>
